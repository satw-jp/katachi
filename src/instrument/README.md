...
11の生成原理へ入る一つの入口。**入口だけ**を持ち、Study 本体を統合しない。

- 表示順は **研究順**（`researchOrder`）。統合の順でも優劣でも完成度でもない
- 各行: 研究順 / 表示名 / 生成原理の一文 / purpose tag / 開くリンク
...
- 見た目は T18 の紙白・墨・hairline token（`src/styles/base.css`）だけを使う。
  新しい design system を作らない。3D canvas も thumbnail も持たない
...
また **root `index.html` は変更していない**。`/` は従来どおり cloud-sculpt で、
launcher は `studies.html` という追加URLとして試す（Q5）。既存9 Study の URL も
redirect も触っていない。

**9 Study の `ui.ts` にある nav-row は今回一切変更していない。**
「各Studyから他8 Studyへ」で数えると66/72で6経路が不足しているが、
