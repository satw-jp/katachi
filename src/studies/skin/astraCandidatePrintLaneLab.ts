...
function refreshTable(): void {
  const body = [...rows.values()].map((row) => {
    ...
    const performance = p ? `profile ${p.support.state}<br>targets ${p.support.targetsProcessed.toLocaleString()} / ${p.support.totalTargets.toLocaleString()}<br>route audits ${p.support.routeAudits.toLocaleString()}<br>support ${p.support.totalMs.toFixed(1)} ms<br>target extraction ${p.support.targetExtractionMs.toFixed(1)} ms<br>target coverage ${p.support.targetCoverageMs.toFixed(1)} ms<br>route generation ${p.support.routeGenerationMs.toFixed(1)} ms<br>route audit ${p.support.routeAuditMs.toFixed(1)} ms<br>vertical audit ${p.support.verticalAuditMs.toFixed(1)} ms<br>leaning audit ${p.support.leaningAuditMs.toFixed(1)} ms<br>spacing ${p.support.spacingMs.toFixed(1)} ms<br>BODY audit ${p.candidateBodyAuditMs.toFixed(1)} ms<br>Rabbit audit ${p.rabbitAuditMs.toFixed(1)} ms<br>BODY SDF ${p.candidateBodySignedDistanceCalls.toLocaleString()}<br>Rabbit SDF ${p.rabbitSignedDistanceCalls.toLocaleString()}<br>BODY nodes avg ${(p.candidateQuery.closestSurfaceCalls ? p.candidateQuery.closestSurfaceNodesVisited / p.candidateQuery.closestSurfaceCalls : 0).toFixed(1)}<br>BODY triangles avg ${(p.candidateQuery.closestSurfaceCalls ? p.candidateQuery.closestSurfaceTrianglesTested / p.candidateQuery.closestSurfaceCalls : 0).toFixed(1)}` : "—";
