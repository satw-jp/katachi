...
async function profileA2(): Promise<void> {
  ...
  try {
    const activated = await send({ type: "ACTIVATE_CANDIDATE", ...nextRequest(), candidateId: "A", filename: row.file.name, file: row.file, placement: deferredPlacement, telemetry: true, queryTimings: true }, "INVENTORY");
