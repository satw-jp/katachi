...
function refreshTable(): void {
  table.innerHTML = `<table><thead><tr><th>Candidate</th><th>Source / execution</th><th>State / placement</th><th>Diagnostics</th><th>Support</th><th>3MF / telemetry</th></tr></thead><tbody>${[...rows.values()].map((row) => { const summary = row.summary; const inventory = row.inventory; const d = summary?.diagnostics; const s = summary?.support; const e = summary?.export; return `<tr><th>${row.id}</th><td>${inventory ? `${inventory.filename}<br>${inventory.sourceByteLength.toLocaleString()} bytes<br>${inventory.sourceSha256.slice(0, 16)}…<br>source ${inventory.triangleCount.toLocaleString()} triangles<br>execution ${inventory.executionTriangleCount.toLocaleString()}<br>near-degenerate ${inventory.nearDegenerateCount}<br>exact-zero ${inventory.exactZeroTriangleCount}<br>removed ${inventory.removedExactZeroSourceFaceIndices.join(",") || "none"}<br>canonicalization ${ASTRA_LARGE_CANDIDATE_CANONICALIZATION_VERSION}<br>${summary ? `execution fingerprint ${summary.geometryFingerprint.slice(0, 16)}…` : "execution fingerprint pending"}` : row.file ? "file reference retained" : "—"}</td><td>${row.state}<br>${inventory ? `SKIN_RUNTIME_TOPOLOGY=${inventory.topologyStatus}<br>ASTRA_ROUND2_EVIDENCE=${inventory.astraRound2Evidence}<br>finite=${inventory.finite}<br>placement ${deferredPlacement ? deferredPlacement.translationMm.z : "pending"}<br>source plate Z ${deferredPlacement ? deferredPlacement.sourcePlateZMm : "pending"}` : "—"}</td><td>${d ? `faces ${d.overhangFaces?.toLocaleString()}<br>regions ${d.overhangRegions}<br>outside ${d.outside?.toLocaleString()}<br>inside ${d.insideExcluded?.toLocaleString()}<br>unresolved ${d.unresolved?.toLocaleString()}<br>reachability invalid ${d.reachabilityInvalidSurfaceTriangles?.toLocaleString()}` : "—"}</td><td>${s ? `critical ${s.critical}<br>supported ${s.supported}<br>unsupported ${s.unsupported}<br>BODY reject ${s.bodyReject}<br>Rabbit reject ${s.rabbitReject}<br>vertical ${s.vertical}<br>offset-bend ${s.offsetBend}<br>nodes ${s.nodes}<br>edges ${s.edges}<br>accepted BODY ${s.acceptedBodyCollision}<br>accepted Rabbit ${s.acceptedRabbitCollision}` : "—"}</td><td>${e ? `validator ${e.validator}<br>${e.archiveBytes.toLocaleString()} bytes<br>support ${e.supportTriangleCount.toLocaleString()} faces<br>placement parity ${e.packagePlacementParity ? "PASS" : "FAIL"}<br>package Z ${e.actualPackageTranslationZ}<br>BODY removed ${e.bodyRemovedDegenerateTriangles}` : "—"}<br>${summary ? `typed resident ${summary.telemetry.residentTypedArrayBytes.toLocaleString()} bytes<br>largest ${summary.telemetry.largestTypedArrayBytes.toLocaleString()} bytes<br>heap ${summary.telemetry.peakJsHeapBytes === null ? "telemetry unavailable" : summary.telemetry.peakJsHeapBytes.toLocaleString()}` : "—"}</td></tr>`; }).join("")}</tbody></table>`;
  refreshActive();
}
...
async function ensureInventories(): Promise<void> { const requiredRows = [...rows.values()]; const missing = requiredRows.filter((row) => !row.file).map((row) => row.id); if (missing.length > 0) throw new Error(`Complete A/G/H/J inventory required before physical comparison; missing ${missing.join(", ")}`); for (const row of requiredRows) if (row.file && !row.inventory) await scanRow(row); const inventories = requiredRows.map((row) => row.inventory).filter((inventory): inventory is LargeCandidateInventory => Boolean(inventory)); if (inventories.length !== 4) throw new Error("A/G/H/J inventory snapshot is incomplete"); commonTranslationZ = -Math.min(...inventories.map((inventory) => inventory.bounds.min.z)); deferredPlacement = makeDeferredPrintPlacement(commonTranslationZ); }
async function processRow(row: Row): Promise<void> {
  if (!row.file) return;
  if (activeCandidate) throw new Error("Another Candidate is active");
  const settings = readSettings(); await ensureInventories(); if (!rabbitReady) throw new Error("Load Rabbit into Worker first"); if (commonTranslationZ === null || !deferredPlacement) throw new Error("Complete candidate placement inventory is not ready");
  activeCandidate = row.id; setRowState(row, "ACTIVE"); refreshActive(); generation += 1;
  try {
    const activated = await send({ type: "ACTIVATE_CANDIDATE", ...nextRequest(), candidateId: row.id, filename: row.file.name, file: row.file, placement: deferredPlacement }, "INVENTORY");
