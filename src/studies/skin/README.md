...
修飾なしの左click／dragは要素選択、直接移動、Support Paintが専有し、TrackballControlsへ渡さない。Shift＋左dragは全viewportでpanとしてcamera操作を優先し、paintや選択を発火させない。Axomeの修飾なし右dragは引き続き連続rotateである。右camera gesture中はbrush hover、stroke、選択を発火させず、pointer captureで開始viewportをdrag終了まで維持する。右drag終了後のcontext menuもcanvasでは抑止する。viewportタイトルのdouble clickはその面の最大化と4面復帰を往復し、最大化buttonとは二重発火させない。
