...
修飾なしの左clickは要素とsupport siteの選択に維持する。Axomeでは修飾なし左dragも4 pxの共通しきい値を超えた時点で連続rotateへ移り、clickとdragを分離する。Support Paintを明示的に有効化している間だけは左dragをbrushへ優先し、Axome左rotateを開始しない。Shift＋左dragは全viewportでpanとしてcamera操作を優先し、paintや選択を発火させない。Axomeの修飾なし右dragも引き続き連続rotateである。camera gesture中はbrush hover、stroke、選択を発火させず、pointer captureで開始viewportをdrag終了まで維持する。右drag終了後のcontext menuもcanvasでは抑止する。viewportタイトルのdouble clickはその面の最大化と4面復帰を往復し、最大化buttonとは二重発火させない。
...
`printApproval=false`を維持する。
## Observation SKIN External STL Host Phase 5 Usagi Repair + V6 Adapter (2026-09-05)

Author-approved `stl-host-boundary-fill-v0` was applied only to rabbit source `C:\dev\samples\rabbit_230223.stl` (SHA-256 `c4d08af61802561ec2adb280d78a928baa00b0c04443a293237706b02cc5afe8`, 10,215,684 bytes). The original source remains immutable and Surface AVAILABLE / Signed Volume UNAVAILABLE `OPEN_BOUNDARY`. With fixed `1 mm/source unit`, `+Y`, right-handed interpretation and uniformScale `20` (2000%), approved loops `0..6` produced a deterministic derived Host with 204,326 valid triangles, zero degenerate triangles, zero boundary edges/loops, zero non-manifold edges, zero orientation inconsistencies, one connected component, Signed Volume AVAILABLE, and explicit self-intersection status `NOT_PROVEN`. Seven degenerate source triangles were removed from the derived mesh and 21 local fan triangles were inserted; derived fingerprint is `90258ce379e3b11aef7e6710ff98ff9f17678a53ae1c7905c3c967bd1e9437d6`.

The V6 adapter reuses the existing `generateShapePoints("flower", …)` path, with deterministic triangle-area-weighted sampling, clearance-aware selection and `GEOMETRIC` placement normals. Motifs remain authored facts and permanent artwork candidates without a `printable` field; the Reference Host is the object that remains `printable=false`. Host transforms are not reapplied to existing motifs. Windows Chrome loaded the actual local rabbit, generated 32 and 128 motifs, and a real coordinate Host OFF check reported `motif positions unchanged: PASS` and `host group only: PASS`, with console error/warning counts at zero. This is a finite geometry/UI capability gate, not slicer, strength or physical-print proof. No FKEI, save/reopen, Astra, Permanent Connection, Web/Internal, Support/FAB, G-code, final BODY or deploy work was performed.

## Observation SKIN External STL Host Phase 6 Persistence / Save-Reopen v0 (2026-09-05)

Phase 6 adds the explicit `katachi.skin.fkei.v2` External Host project envelope without changing `katachi.skin.fkei.v1`. The v2 document embeds the exact original STL bytes, source SHA-256/byte length/filename/format/interpretation, the instance transform, approved repair policy and loop list, repair parameters, expected repaired-mesh fingerprint and capability facts. Authored V6 motifs retain exact points, ids, shape parameters, Host placement, authored transform, adapter version and `GEOMETRIC` normal policy. The Reference Host is `printable=false`; authored motifs are permanent artwork candidates and have no `printable` field. Repaired mesh authority, BVH/cache and Three objects are not serialized.

On reopen, v2 rehashes the embedded bytes, recreates the original Host from those bytes only, reapplies the approved repair, compares the expected repaired fingerprint and capability facts, and restores the exact authored motifs without regeneration. Hash, byte-length, policy, repair, capability or motif-contract failures are fail-closed; hydration is completed before live replacement, and a replacement failure rolls back the previous live state. The actual rabbit browser gate saved 14,761,406 JSON bytes containing 10,215,684 STL bytes, reopened 128 motifs from embedded bytes with source-path requirement `NO`, passed source rehash, repair fingerprint, exact motif geometry and Signed Volume checks, then toggled Host OFF with motif positions unchanged and Host group only. Chrome console errors/warnings were `0/0`. This is a finite geometry/persistence capability gate, not slicer, strength or physical-print proof. No Astra, Permanent Connection, Web/Internal, Support/FAB, G-code, final BODY or deploy work was performed.

## Observation SKIN External Host v2 actual file export (2026-09-05)

The V6 Author Gate now has separate `Download .fkei` and `Open .fkei` controls while retaining the existing in-memory `Save embedded v2 project` / `Reopen embedded bytes` gate. Download captures the current External Host v2 document with the existing `captureExternalHostProject()` and `serializeExternalHostProject()` path, creates a normal browser Blob, and downloads the exact `katachi.skin.fkei.v2` JSON text as `usagi-v6-512-varied.fkei`. The actual Chrome file was `18,357,594` bytes and contained the embedded `10,215,684`-byte rabbit source, source SHA-256 `c4d08af61802561ec2adb280d78a928baa00b0c04443a293237706b02cc5afe8`, repair fingerprint `90258ce379e3b11aef7e6710ff98ff9f17678a53ae1c7905c3c967bd1e9437d6`, 512 varied motifs, base size `2.4`, variance `0.35`, and Host visibility `ON`.

Opening that downloaded file through the new `.fkei` file input reads text and routes it through the existing atomic v2 hydrate path. Chrome reported Open PASS, source hash PASS, repair fingerprint PASS, Signed Volume AVAILABLE, exact motif geometry PASS, 512 motifs restored, Host visibility restored ON with no external source path, then Host OFF reported motif positions unchanged PASS and host group only PASS. The existing in-memory Save/Reopen gate also passed after file Open. Console errors/warnings were `0/0`. v1, v2 schema, golden-ratio placement, repair, Support/FAB and Permanent structure were unchanged; no deploy was performed.

## Observation SKIN Astra Large Candidate Execution v0 (2026-09-06)

The Astra large-candidate lane now keeps candidate File references and compact summaries on the main thread, transfers one active Candidate at a time to a dedicated Worker, streams binary STL chunks through an incremental SHA-256, retains packed Float32 positions, uses a typed-array Candidate query, and releases the active packed state before the next Candidate. The Rabbit Reference Host remains the existing approved repaired Host: source SHA-256 `c4d08af61802561ec2adb280d78a928baa00b0c04443a293237706b02cc5afe8`, repair fingerprint `90258ce379e3b11aef7e6710ff98ff9f17678a53ae1c7905c3c967bd1e9437d6`, Signed Volume AVAILABLE, and uniformScale `20`.

The real Chrome gate loaded A2_BODY.stl (`296,702,384` bytes, SHA-256 `2030a945eb44fb3a263c667305f10ce8a773af5d8914cfca82d7c3f68680b04c`, `5,934,046` triangles, finite=true), completed packed activation and reached Overhang detection (`839,603` faces). The first exact bounded execution blocker was the existing fail-closed Support reachability contract: the final Surface occlusion mesh contained `8` invalid faces. This is a measured malformed/degenerate-surface blocker, not a Worker memory-exhaustion result; G2/H2/J2 were not started. Runtime topology remains `NOT_RECOMPUTED`, while the inventoried Astra Round-2 evidence remains `PASS`. The browser gate displayed console errors/warnings `0/0`; Support, 3MF export, release, physical comparison and production integration were not claimed.

## Observation SKIN Large Candidate 3MF Streaming Export v0 (2026-09-06)

The previous A2 export failed before compression because `meshXml()` retained millions of element strings and joined the complete `/3D/Objects/object_1.model` into one JavaScript string. The large-object path now preserves `indexTriangleSoup()` and exact XML order while encoding at most 1 MiB of ASCII XML per input chunk, feeding those chunks into streaming raw-deflate, and computing CRC32 and ZIP32 sizes incrementally. Small BODY-only and BODY plus separate PRINT_SUPPORT fixtures retained byte-identical uncompressed XML entries, placement/model-settings parity and validator PASS. A deterministic synthetic object with 1,000,002 vertices and 13,200,000 triangles produced 538,792,408 XML bytes, a 5,049,187-byte compressed entry and a 5,049,335-byte ZIP with a 1,048,576-byte maximum serialization chunk and no `Invalid string length`.

The actual A2 source (`2030a945eb44fb3a263c667305f10ce8a773af5d8914cfca82d7c3f68680b04c`) retained 5,934,044 execution faces, completed unchanged Sparse Support at 4,561/4,561 targets, 654 supported, 3,907 unsupported, 2,150 nodes, 1,496 edges and accepted BODY/Rabbit collision 0/0, then advanced through Support mesh, BODY/Support indexing, XML streaming, compression and ZIP assembly. The next exact blocker is the existing validator's complete-entry materialization: after observing at least 67,637,175 compressed model bytes, `validateSkin3mf()` returned `XML has no root element` for the large object model and downstream missing-object/mesh errors. Per the bounded task rule, validator redesign, download/release and G/H/J were not performed. Browser console errors/warnings remained 0/0; geometry, Support semantics/settings, Rabbit, FKEI, authoring round-trip and production SKIN were unchanged, and no deploy was performed by explicit task instruction.

## Observation SKIN Large 3MF Streaming Validator v0 (2026-09-06)

The large object-model validator now reads ZIP central/local metadata into lazy entry descriptors, streams only large model entries through raw-deflate, fatal incremental UTF-8 decoding and an event parser, and retains compact per-object counts/bounds/errors rather than the full model bytes, JavaScript string, XML tree or per-vertex objects. CRC32 and uncompressed size are checked incrementally. Forced legacy and streaming execution produced deeply identical complete reports for valid BODY/BODY+Support packages and malformed index, non-finite coordinate, truncated XML, invalid UTF-8 and CRC fixtures at 7, 31 and 64-byte compressed-input boundaries. A synthetic 1,000,002-vertex / 13,200,000-triangle model validated 538,792,408 XML bytes from a 5,049,187-byte compressed entry with a 1,048,576-byte maximum compressed-input chunk and a 16,414-character maximum parser buffer.

The unchanged real A2 run completed 4,561/4,561 Support targets with 654 accepted, 3,907 unsupported, 2,150 nodes, 1,496 edges and accepted BODY/Rabbit collision 0/0. Its 3MF contained 2,964,210 BODY vertices, 5,934,044 BODY faces and 71,808 separate support faces; the model XML was 575,613,609 bytes, compressed model 75,489,493 bytes and archive 75,491,879 bytes. Streaming validation passed, package translation Z matched 48.029293060302734, BODY indexing removed zero faces, download and candidate release passed, and browser console errors/warnings remained 0/0. Candidate geometry, Support semantics/settings, Rabbit, 3MF schema, FKEI, authoring round-trip and production SKIN were unchanged. G/H/J were not run, no winner was selected, no merge was performed, and no deploy was performed by explicit task instruction.

## Observation SKIN Candidate Artifact Retention / Checkpoint v0 (2026-09-07)

The actual Windows Chrome A2 retention gate used the explicitly selected temporary output directory `J:/My Drive/codex/2026-09-05/files-pasted-by-the-user-katachi/browser-retention-gate-20260907`. A2_BODY.stl was `296,702,384` bytes with source SHA-256 `2030a945eb44fb3a263c667305f10ce8a773af5d8914cfca82d7c3f68680b04c`; unchanged Full Sparse Support completed `4,561/4,561` targets with `654` supported, `3,907` unsupported, `2,150` nodes, `1,496` edges and accepted BODY/Rabbit collision `0/0`. Existing validation and candidate release passed. The retained `ASTRA_A_candidate-print-lane.3mf` is `75,491,874` bytes with SHA-256 `de304365a3247487f7ec18db1536d2234e9980a57576d6acffb0aa3c00460874`; `ASTRA_A_candidate-print-lane.evidence.json` is `7,971` bytes. The archive was written, closed, reopened and matched in exact byte length, byte content and SHA-256 (`durableVerification PASS`); the sidecar was saved and read back. The focused retention fixture, both TypeScript checks, production build and `git diff --check` passed. This update records retained evidence only; A2 was not rerun here, and G/H/J, Performance v2, new Support architecture, geometry, FKEI, merge and deploy were not started.
