...
Candidate print-lane release now requires an explicitly authorized browser output directory. The existing validated 3MF archive is written to a deterministic `ASTRA_<candidate>_candidate-print-lane.3mf` filename, closed, reopened through the same directory handle, and checked for exact byte length, byte content and SHA-256 before the existing Worker `RELEASE_CANDIDATE` message is sent. A matching `.evidence.json` sidecar is then written and read back; it records the compact candidate, diagnostics, Support, Rabbit, placement, export, validator and runtime facts while excluding the binary archive. A mismatch fails closed before sidecar/release. The focused filesystem fixture passed write/close/reopen/bytes/SHA, sidecar identity and mismatch handling; both TypeScript checks, production build and `git diff --check` passed. No user-selected browser output directory was authorized in this session, so the real Chrome persistence gate remains UNVERIFIED. A2 was not rerun; G/H/J, Performance v2, new Support architecture, geometry, FKEI, merge and deploy were not started.
