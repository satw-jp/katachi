...
## Observation SKIN Support v2 Experimental Shared Trunk / Branched Tree v0（2026-09-04）

「1 target = 1 trunk」の前提を拡張し、複数targetが下部経路を共有できる場合にtemporary branched tree（targets→branches→shared trunk→plate）を組めるか実験した。Footing experimentの知見（太くして耐えるより早く相互支持へ入る方が効率的）を進め、Shared Trunk≠Crown≠Mutual Braceの区別を保つ。直前branch（bootstrap-footing @0055612）をbaseにし、bootstrap metric・junction・low brace・safety分析を再利用した。Print #2 candidateはFROZENのまま触らず、Output Scale branchは混ぜていない。

純粋module `supportBranchedTree.ts`がindividual-route baselineの上でshared corridorを採点し（target近接だけではshareしない）、shared trunk＋branch junction（BODY 0・有限・決定的・由来記録つき、childは既存route上部を再利用）を建てる。failure-domain記録とcritical cap（初期4 targets / 2 critical、production ruleではない）で無制限集中を禁じ、原則TREE（cycleなし）を保つ。trunk径は現行support径のまま（multiplier契約default 1.0、未接続）。Synthetic fixture 5 targetではIndependent（trunk 5・conflict 1・bootstrap max 36.8）→Shared（tree {a+b}＋solo 3・conflict解決1・新規0・branch最大10.6°・材料−8%・安全0・failure domain 2/0）となり、発散ペアは角度72°でrejectされた。Shared+LowDiagonalはbraceを証拠として合成しbootstrapを悪化させない。表示は独立viewerで行いmode・layer切替は表示切替のみ（mutation 0）。focused test 14件と既存回帰・tsc・buildは通り、実ブラウザ確認は作者機WAITING。有機形状は作者が作るものを受け取る段階まで生成しない。`printApproval=false`を維持する。

## Observation SKIN Support v2 Multi-Fixture / Author Organic Fixture Intake v0（2026-09-04）

Support v2をSynthetic垂直fixtureだけへ最適化し続けないため、Synthetic Vertical Stress Fixtureを回帰用に保持したまま、作者の有機形状を後から読み込む受け皿を作った。Author形状そのものは生成せず（proceduralな有機sampleは作らない。unit testの不規則数値はTEST FIXTUREと明記）、空slotはNOT LOADED表示で、未提供をerrorにもsynthetic fallback生成にもしない。

fixture contract（`katachi.support-experiment-fixture.v1`、FKEIとは別・FKEI schema不変）はderived evidenceだけを運び、finite・ID重複・segment ref・physical値・BODY evidence・schema版をfail closed検証する。BODY triangle soupはexact unsigned distanceで監査し（v0上限5000 triangles、複数componentをinvalidにしない）、unknown scaleはnullのままround-tripし、import時のsilent rescaleを禁じた。Synthetic adapterは7adb9f2 baselineとbyte同一（independent 5/5/0/1・max 36.8・mean 27.76・conflict 1、shared 1 tree・2 branches・10.62°・解決1・新規0・−8.0%）。registryはSynthetic＋Author 1枠で切替決定的、同一parameter・同一algorithm pathで3 modeを実行し、cross-fixture比較と観察専用warning（自動FAILなし）を出す。capture adapterはpureで入力不変・production無接触（Golden UIへの配線は設計報告のみ）。viewer `skin-support-v2-fixture-lab.html`でfixture切替・local load・BODY/Targets/Routes等の10 toggle・metrics/provenance/comparison/warningsを表示する。focused test 15件と既存回帰・tsc・buildは通り、実ブラウザのA–J確認は作者機WAITING。Print #2はFROZENのまま。`printApproval=false`を維持する。
