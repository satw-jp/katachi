...
const booleanSelect = (path: string, label: string, off = "OFF", on = "ON"): string => `<label class="composer-control"><span>${label}</span><select data-control="${path}"><option value="false">${off}</option><option value="true">${on}</option></select></label>`;
...
function updateControl(path: string, rawValue: string): void {
  ...
  const value = typeof current === "number" ? Number(rawValue) : typeof current === "boolean" ? rawValue === "true" : rawValue;
