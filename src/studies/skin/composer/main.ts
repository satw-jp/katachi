...
function manifest(): Record<string, unknown> { return { format: "katachi.skin-art.composer.v1", sourceFingerprint: source?.fingerprint ?? "unavailable", seed: state.seed, state, timeSeconds: runtime?.timeSeconds ?? 0, camera: runtime?.cameraData() ?? null, budget: runtime?.budgetData ?? null }; }
...
function copyLink(): void { const url = new URL(serializeComposerState(window.location.href, state)); url.searchParams.delete("preset"); void navigator.clipboard?.writeText(url.toString()); setStatus("LINK COPIED / FIXED STATE"); }
