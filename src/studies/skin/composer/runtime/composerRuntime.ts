...
const SPRITE_VERTEX = /* glsl */ `
  ...
  void main() {
    ...
    animated += aDrift * uDrift * (0.4 + driftDecision * 0.46 + wave * 0.68 + pause * 0.12) * uElementMotionScale;
    animated += normalize(displayPosition + aDrift * 0.2 + vec3(0.001)) * hesitation * 0.019 * uElementMotionScale;
    animated += normalize(displayPosition + vec3(0.001)) * oscillation * uOscillation * (0.008 + aShape.x * 0.022) * uElementMotionScale;
    animated -= aDrift * residue * (0.8 + aMetrics.z * 1.6) * uElementMotionScale;
    animated += normalize(aDrift + vec3(0.001)) * growthReveal * uGrowth * (0.045 + aMetrics.w * 0.045) * uElementMotionScale;
    ...
    gl_PointSize = clamp(aSize * uSize * uSplatScale * depthScale * perspective * (0.76 + growth * 0.3 + abs(oscillation) * uOscillation * 0.1), 1.0, 260.0);
    float emission = uStyle < 0.5 ? 0.82 : mix(0.62, 1.0, clamp(uLightAccumulation / 2.0, 0.0, 1.0));
    vAlpha = aAlpha * uAmount * densityGate * emission * (0.62 + 0.38 * pause) * (0.7 + aMetrics.x * 0.3) * (0.78 + growth * 0.22);
    vPhase = aPhase;
    vAspect = aShape.y;
...
const SPRITE_FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    float whiteResult = smoothstep(0.69, 1.0, vStyleMetric * (0.55 + uLightAccumulation * 0.45)) * uHighlight;
...
function spriteLayer(data: readonly SpriteDatum[], style: number): SpriteLayer {
  ...
  const material = new THREE.ShaderMaterial({ vertexShader: SPRITE_VERTEX, fragmentShader: SPRITE_FRAGMENT, transparent: true, depthTest: true, depthWrite: false, blending: THREE.AdditiveBlending, uniforms: { uTime: { value: 0 }, uAmount: { value: 0 }, uSize: { value: 1 }, uDensity: { value: 1 }, uFieldCompression: { value: 0 }, uSplatScale: { value: 1 }, uLightAccumulation: { value: 1 }, uElementMotionScale: { value: 1 }, uOpacity: { value: style === 3 ? 0.72 : 1 }, uStyle: { value: style }, uDrift: { value: 0 }, uWave: { value: 0 }, uGrowth: { value: 0 }, uTremor: { value: 0 }, uAccumulation: { value: 0 }, uOscillation: { value: 0 }, uDepthSpread: { value: 1 }, uForegroundScale: { value: 1.2 }, uBackgroundScale: { value: 0.8 }, uFocusDisorder: { value: 0.65 }, uSpatialEcho: { value: 0.55 }, uParallax: { value: 0.3 }, uFieldCenter: { value: new THREE.Vector3() }, uHighlight: { value: 0.72 }, uBlackRetention: { value: 0.65 }, uSaturation: { value: 0.8 }, uWaveOrigins: { value: Array.from({ length: 6 }, () => new THREE.Vector3()) }, uWaveParams: { value: Array.from({ length: 6 }, () => new THREE.Vector4()) } }, name: `skin-art-composer-sprite-${style}` });
...
export class ComposerRuntime {
  ...
  restart(): void { this.elapsedSeconds = 0; this.lastTick = performance.now(); this.lastAutoRotateSeconds = 0; this.autoRotateAngle = 0; this.playing = true; }
  resetCamera(): void { this.clearAutoOffset(); this.autoRotateAngle = 0; this.lastAutoRotateSeconds = this.elapsedSeconds; this.camera.position.copy(this.defaultCameraPosition); this.camera.up.copy(this.defaultCameraUp); this.controls.target.copy(this.defaultCameraTarget); this.camera.fov = this.state.camera.fov; this.controls.update(); this.captureUserPose(); }
  ...
  private createLayers() {
    ...
    return { points: spriteLayer(expandDensityData(pointData, this.state.seed), 0), gaussian: spriteLayer(expandDensityData(gaussianData, this.state.seed + 101), 1), cloud: spriteLayer(expandDensityData(cloudData.slice(0, this.budget.cloud), this.state.seed + 211), 2), light: spriteLayer(expandDensityData(lightData.slice(0, Math.max(1, Math.floor(this.budget.cloud * 0.72))), this.state.seed + 307), 2), void: spriteLayer(expandDensityData(voidData, this.state.seed + 401), 3), hairlines: lineLayer(this.source, palette), softLines: spriteLayer(expandDensityData(softLineData, this.state.seed + 503), 1) };
  }
  ...
  private updateLines(layer: LineLayer, amount: number, blur: number): void {
    ...
    this.source.edges.forEach((edge, index) => { const wave = this.waveAt(edge.midpoint); const localPhase = fract(this.elapsedSeconds * (0.16 + (index % 5) * 0.012) - (index % 11) * 0.08); const growthReveal = this.state.motion.growth <= 0 ? 1 : smoothReveal(localPhase); const wobble = Math.sin(this.elapsedSeconds * (0.38 + edge.directionChange * 1.1) + index * 1.7) * this.state.motion.tremor * edge.directionChange * 0.018 * motionScale; const drift = edge.direction.clone().multiplyScalar((this.state.motion.drift * (0.01 + edge.supportRole * 0.016) + wave * this.state.motion.wave * 0.008) * motionScale); const start = layer.starts[index]!.clone().sub(this.cameraTargetOrigin).multiplyScalar(fieldScale).add(this.cameraTargetOrigin).add(drift).add(new THREE.Vector3(0, 0, wobble)); const fullEnd = layer.ends[index]!.clone().sub(this.cameraTargetOrigin).multiplyScalar(fieldScale).add(this.cameraTargetOrigin).add(drift).add(new THREE.Vector3(0, 0, wobble)); const end = start.clone().lerp(fullEnd, growthReveal); const offset = index * 6; values[offset] = start.x; values[offset + 1] = start.y; values[offset + 2] = start.z; values[offset + 3] = end.x; values[offset + 4] = end.y; values[offset + 5] = end.z; }); positions.needsUpdate = true;
  }
  ...
  private readonly handleControlStart = (): void => { this.clearAutoOffset(); this.autoRotateAngle = 0; this.lastAutoRotateSeconds = this.elapsedSeconds; this.userInteracting = true; this.autoRotateResumeAt = Number.POSITIVE_INFINITY; this.controls.update(); this.captureUserPose(); };
  ...
  private updateCamera(): void {
    const t = this.elapsedSeconds; const camera = this.state.camera; const now = performance.now(); this.clearAutoOffset(); this.controls.update(); this.captureUserPose(); this.camera.fov = camera.fov;
    this.cameraPositionOffset.set(0, 0, 0); this.cameraTargetOffset.set(0, 0, 0);
    if (camera.mode === "DRIFT") {
      this.cameraPositionOffset.set(Math.sin(t * 0.11) * camera.orbit * 0.22, Math.cos(t * 0.08) * camera.dolly * 0.26, Math.sin(t * 0.07) * camera.targetShift * 0.18);
      this.cameraTargetOffset.set(Math.sin(t * 0.09) * camera.targetShift * 0.2, Math.cos(t * 0.067) * camera.targetShift * 0.14, Math.sin(t * 0.053) * camera.targetShift * 0.1);
    } else if (camera.mode === "EXPLORE") {
      const direction = this.cameraAxis.copy(this.userPosition).sub(this.userTarget).normalize();
      this.cameraPositionOffset.set(Math.sin(t * 0.071) * camera.orbit * 0.5 + Math.cos(t * 0.037) * camera.dolly * 0.24, Math.cos(t * 0.059) * camera.dolly * 0.42, Math.sin(t * 0.047) * camera.targetShift * 0.34);
      this.cameraPositionOffset.addScaledVector(direction, Math.sin(t * 0.041) * camera.passThrough * 0.36);
      this.cameraTargetOffset.set(Math.sin(t * 0.061) * camera.targetShift * 0.34, Math.cos(t * 0.043) * camera.targetShift * 0.24, Math.sin(t * 0.029) * camera.targetShift * 0.18);
    }
    const autoRotate = camera.autoRotate && !this.userInteracting && now >= this.autoRotateResumeAt;
    if (autoRotate) {
      const elapsedDelta = Math.max(0, this.elapsedSeconds - this.lastAutoRotateSeconds);
      const modeFactor = camera.mode === "EXPLORE" ? 0.58 : 1;
      const direction = camera.autoRotateDirection === "CCW" ? -1 : 1;
      this.autoRotateAngle += elapsedDelta * THREE.MathUtils.degToRad(camera.autoRotateSpeed) * direction * modeFactor;
    }
    this.lastAutoRotateSeconds = this.elapsedSeconds;
    this.cameraPositionWork.copy(this.userPosition).add(this.cameraPositionOffset);
    this.cameraTargetWork.copy(this.userTarget).add(this.cameraTargetOffset);
    if (autoRotate) this.cameraPositionWork.sub(this.userTarget).applyAxisAngle(this.cameraAxis.set(0, 0, 1), this.autoRotateAngle).add(this.userTarget);
