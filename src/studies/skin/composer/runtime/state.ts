...
export type ComposerCameraMode = "STILL" | "DRIFT" | "EXPLORE";
export type ComposerColorSource = "MOTIF" | "DENSITY" | "CONNECTIVITY" | "DIRECTION" | "SUPPORT";

export interface ComposerState {
  readonly seed: number;
  readonly visual: { points: number; gaussian: number; hairlines: number; softLines: number; cloud: number; void: number };
  ...
  readonly camera: { mode: ComposerCameraMode; dolly: number; orbit: number; targetShift: number; passThrough: number; fov: number };
...
export const DEFAULT_COMPOSER_STATE: ComposerState = {
  seed: 12345,
  visual: { points: 0.64, gaussian: 0.82, hairlines: 0.26, softLines: 0.48, cloud: 0.58, void: 0.14 },
  ...
  camera: { mode: "DRIFT", dolly: 0.12, orbit: 0.1, targetShift: 0.16, passThrough: 0.08, fov: 46 },
