...
- **A1 mini PETG+PLA:** separate bounded physical lane. PHYSICAL_01 remains bound/LOW/no PASS. The old direct-G-code dry-only path is no longer the active physical action. Author-visible 3MF V2 Attempt A was stopped after the Author observed that the running job had no raft; it is INVALID for the intended V2 comparison. V2 Attempt B was restarted and its physical result is pending. Exact restarted raft/payload/overrides still need binding.
...
2. **A1 mini Author-visible V2 Attempt B physical review:** bind actual raft condition / payload / overrides and review bed-on, before-removal and after-removal photos. Attempt A stays invalid because raft was absent; do not merge attempts.
