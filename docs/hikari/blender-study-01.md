...
The equal IOR and zero inclusion absorption captured the intended relationship, but v0.29.1 still modelled the clear region as a hard analytic medium and substituted a `27.3°` sun disc for the source. That case remains useful evidence of the failed hypothesis, not the Blender parity baseline.

The supplied `hikari-2026-08-01T08-39-10-808Z.hkr` case also reveals a setup miss rather than only a transport failure: its camera centre ray is `1.114°` from the sun centre while the authored `0.8°` sun has a `0.4°` radius. With sky intensity `0.1` and host absorption `27.3`, that ray correctly samples the dark sky instead of the bright disc. Hikari therefore provides a one-action camera alignment that preserves target and distance and reports centre separation versus sun radius. This is a reproducible input baseline, not a promise that the sun remains centred after the transparent body: for this irregular outer form the centre BODY ray moves roughly `5–7°` after geometric/cosmetic refraction, so a physical `0.53–0.8°` sun can still be missed. The paired `27.3°` case is a broad-source diagnostic, not a physical sun and not the final Blender comparison.

v0.29.2 re-read the active Blender graph and corrected three mismatches. The active target is a gray World plus a finite 0.8-by-0.8 Emission plane; Area and Point lights are hidden. The absorption density is at most 0.5 and falls smoothly to zero in the Object-coordinate region while IOR remains 1.45. Hikari now applies its missing linear-to-output display transform, collapses equal-IOR nested boundaries into a sampled smooth absorption void for BODY, and offers a saved finite rectangular backlight emitter. `hikari-blender-backlight-study.hkr` records this corrected case. The emitter is deliberately BODY-only until receiver transport accepts the same finite source. It also remains separate from the author's physical west-sun-through-window reference, which requires room and aperture geometry rather than a view-only source proxy.
